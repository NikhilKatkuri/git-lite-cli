...
*.pfx`
    ;
export async function GitPushCmd(repoUrl, rootPath, commitMsg) {
...
async function GitPush() {
    const { repoName, repopath, commit } = await inquirer.prompt([
        {
            name: "repoName",
            type: "input",
            message: "Enter a correct repo URL (https://github.com/****/*.git) or 'y' to use stored remote:",
            default:"y"
        },
        {
            name: "repopath",
    ...
    if (repoName === "y") {
        if (!fs.existsSync(tracePath)) {
            console.log(chalk.red("❌ No stored remote found. Please enter the repo URL."));
            return;
        }
        const readTrace = fs.readFileSync(tracePath, "utf-8");
        const remoteUrl = SearchAsArray(readTrace, "remote");
        if (!remoteUrl) {
